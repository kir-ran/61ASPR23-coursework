...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # location container ant or a ant,
            # BEGIN Problem 8b
            ...
            # container ant always gets priority in place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # Place.bees has as list of bees
        place = 0
        self.place_check = self.place
        while not self.place_check.is_hive:
            if not self.place_check.is_hive and self.place_check.bees and self.lower_bound <= place <= self.upper_bound:
                return random_bee(self.place_check.bees)

                # loop through all places, starting at the current place
            self.place_check = self.place_check.entrance
            place += 1

        # if there are bees in the current place:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # for bee in your location
        for bee in stored_place.bees[:]:
...
class HungryAnt(Ant):
    ...
    # while chewing, do jack shit

    def action(self, gamestate):
        if self.turns_to_chew != 0:  # if it's chewing
            self.turns_to_chew -= 1

        elif not self.place.bees:  # if it's not chewing and there's a bee
            return
        else:
            poor_victim = random_bee(self.place.bees)
            poor_victim.reduce_health(poor_victim.health)
