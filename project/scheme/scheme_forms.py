...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        check_formals = signature.rest
        while check_formals is not nil:
            if isinstance(check_formals.first, int):
                raise SchemeError('non-symbol: {0}'.format(check_formals))
            check_formals = check_formals.rest
...
def do_and_form(expressions, env):
    ...
    elif is_scheme_false(scheme_eval(expressions.first, env)):  # if false, return false
        return scheme_eval(expressions.first, env)
    else:
