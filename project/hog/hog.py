...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    top = 0
    while x <= 10:
        average_dice = make_averaged(roll_dice, total_samples)(x, dice)
        if average_dice > top:
            top = average_dice
            result = x
        x += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # pigtail - tail strat = tail_strategy(score, opponent_score, threshold, num_rolls) RETURNS NUMBER OF ROLLS
    # TAIL_POINTS RETURNS SCORE IF YOU ROLL 0 DICE
    result = 0
    if perfect_square(tail_points(opponent_score) + score):
        # square_update(num_rolls, (tail_points(opponent_score) + score), opponent_score, dice=six_sided)
        result = next_perfect_square(tail_points(opponent_score) + score)
    else:
    ...
    if result - score >= threshold:
        return 0
    return num_rolls
    # if square_update(num_rolls, score, opponent_score, dice=six_sided) - score >= threshold:
    #    return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
